...
func (_q *MenuQuery) QueryItems() *MenuItemQuery {
...
func (_q *MenuQuery) WithItems(opts ...func(*MenuItemQuery)) *MenuQuery {
	query := (&MenuItemClient{config: _q.config}).Query()
...
func (_q *MenuQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Menu, error) {
	...
	if query := _q.withItems; query != nil {
		if err := _q.loadItems(ctx, query, nodes,
			func(n *Menu) { n.Edges.Items = []*MenuItem{} },
			func(n *Menu, e *MenuItem) { n.Edges.Items = append(n.Edges.Items, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MenuQuery) loadItems(ctx context.Context, query *MenuItemQuery, nodes []*Menu, init func(*Menu), assign func(*Menu, *MenuItem)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(menuitem.FieldMenuID)
	}
	query.Where(predicate.MenuItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(menu.ItemsColumn), fks...))
	}))
