...
type MediaQuery struct {
	...
	withTranslations *MediaTranslationQuery
...
func (_q *MediaQuery) QueryTranslations() *MediaTranslationQuery {
	query := (&MediaTranslationClient{config: _q.config}).Query()
...
func (_q *MediaQuery) WithTranslations(opts ...func(*MediaTranslationQuery)) *MediaQuery {
	query := (&MediaTranslationClient{config: _q.config}).Query()
...
func (_q *MediaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Media, error) {
	...
	if query := _q.withTranslations; query != nil {
		if err := _q.loadTranslations(ctx, query, nodes,
			func(n *Media) { n.Edges.Translations = []*MediaTranslation{} },
			func(n *Media, e *MediaTranslation) { n.Edges.Translations = append(n.Edges.Translations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MediaQuery) loadTranslations(ctx context.Context, query *MediaTranslationQuery, nodes []*Media, init func(*Media), assign func(*Media, *MediaTranslation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mediatranslation.FieldMediaID)
	}
	query.Where(predicate.MediaTranslation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(media.TranslationsColumn), fks...))
	}))
