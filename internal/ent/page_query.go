...
type PageQuery struct {
	...
	withTranslations *PageTranslationQuery
...
func (_q *PageQuery) QueryTranslations() *PageTranslationQuery {
	query := (&PageTranslationClient{config: _q.config}).Query()
...
func (_q *PageQuery) WithTranslations(opts ...func(*PageTranslationQuery)) *PageQuery {
	query := (&PageTranslationClient{config: _q.config}).Query()
...
func (_q *PageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Page, error) {
	...
	if query := _q.withTranslations; query != nil {
		if err := _q.loadTranslations(ctx, query, nodes,
			func(n *Page) { n.Edges.Translations = []*PageTranslation{} },
			func(n *Page, e *PageTranslation) { n.Edges.Translations = append(n.Edges.Translations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PageQuery) loadTranslations(ctx context.Context, query *PageTranslationQuery, nodes []*Page, init func(*Page), assign func(*Page, *PageTranslation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(pagetranslation.FieldPageID)
	}
	query.Where(predicate.PageTranslation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(page.TranslationsColumn), fks...))
	}))
