...
type TransactionQuery struct {
	...
	withSession *PaymentSessionQuery
...
func (_q *TransactionQuery) QuerySession() *PaymentSessionQuery {
...
func (_q *TransactionQuery) WithSession(opts ...func(*PaymentSessionQuery)) *TransactionQuery {
	query := (&PaymentSessionClient{config: _q.config}).Query()
...
//		GroupBy(transaction.FieldCreatedBy).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Transaction.Query().
//		Select(transaction.FieldCreatedBy).
//		Scan(ctx, &v)
...
func (_q *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := _q.withSession; query != nil {
		if err := _q.loadSession(ctx, query, nodes, nil,
			func(n *Transaction, e *PaymentSession) { n.Edges.Session = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TransactionQuery) loadSession(ctx context.Context, query *PaymentSessionQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *PaymentSession)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		fk := nodes[i].PaymentSessionID
	...
	query.Where(paymentsession.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "payment_session_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TransactionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSession != nil {
			_spec.Node.AddColumnOnce(transaction.FieldPaymentSessionID)
		}
	}
