...
func (_q *OrderQuery) QueryFulfillments() *FulfillmentQuery {
...
func (_q *OrderQuery) QueryReturns() *OrderReturnQuery {
...
func (_q *OrderQuery) WithFulfillments(opts ...func(*FulfillmentQuery)) *OrderQuery {
	query := (&FulfillmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) WithReturns(opts ...func(*OrderReturnQuery)) *OrderQuery {
	query := (&OrderReturnClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := _q.withFulfillments; query != nil {
		if err := _q.loadFulfillments(ctx, query, nodes,
			func(n *Order) { n.Edges.Fulfillments = []*Fulfillment{} },
			func(n *Order, e *Fulfillment) { n.Edges.Fulfillments = append(n.Edges.Fulfillments, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withReturns; query != nil {
		if err := _q.loadReturns(ctx, query, nodes,
			func(n *Order) { n.Edges.Returns = []*OrderReturn{} },
			func(n *Order, e *OrderReturn) { n.Edges.Returns = append(n.Edges.Returns, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OrderQuery) loadFulfillments(ctx context.Context, query *FulfillmentQuery, nodes []*Order, init func(*Order), assign func(*Order, *Fulfillment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(fulfillment.FieldOrderID)
	}
	query.Where(predicate.Fulfillment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.FulfillmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *OrderQuery) loadReturns(ctx context.Context, query *OrderReturnQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderReturn)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(orderreturn.FieldOrderID)
	}
	query.Where(predicate.OrderReturn(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.ReturnsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
