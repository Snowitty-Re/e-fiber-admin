...
type CollectionQuery struct {
	...
	withTranslations *CollectionTranslationQuery
...
func (_q *CollectionQuery) QueryTranslations() *CollectionTranslationQuery {
	query := (&CollectionTranslationClient{config: _q.config}).Query()
...
func (_q *CollectionQuery) QueryProducts() *ProductQuery {
...
func (_q *CollectionQuery) WithTranslations(opts ...func(*CollectionTranslationQuery)) *CollectionQuery {
	query := (&CollectionTranslationClient{config: _q.config}).Query()
...
func (_q *CollectionQuery) WithProducts(opts ...func(*ProductQuery)) *CollectionQuery {
	query := (&ProductClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(collection.FieldCreatedBy).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Collection.Query().
//		Select(collection.FieldCreatedBy).
//		Scan(ctx, &v)
...
func (_q *CollectionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Collection, error) {
	...
	if query := _q.withTranslations; query != nil {
		if err := _q.loadTranslations(ctx, query, nodes,
			func(n *Collection) { n.Edges.Translations = []*CollectionTranslation{} },
			func(n *Collection, e *CollectionTranslation) { n.Edges.Translations = append(n.Edges.Translations, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withProducts; query != nil {
		if err := _q.loadProducts(ctx, query, nodes,
			func(n *Collection) { n.Edges.Products = []*Product{} },
			func(n *Collection, e *Product) { n.Edges.Products = append(n.Edges.Products, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CollectionQuery) loadTranslations(ctx context.Context, query *CollectionTranslationQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *CollectionTranslation)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(collectiontranslation.FieldCollectionID)
	}
	query.Where(predicate.CollectionTranslation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(collection.TranslationsColumn), fks...))
	}))
...
func (_q *CollectionQuery) loadProducts(ctx context.Context, query *ProductQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *Product)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Collection)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Product(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(collection.ProductsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.collection_products
		if fk == nil {
			return fmt.Errorf(`foreign-key "collection_products" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "collection_products" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
