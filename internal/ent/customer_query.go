...
func (_q *CustomerQuery) QueryAddresses() *CustomerAddressQuery {
	query := (&CustomerAddressClient{config: _q.config}).Query()
...
func (_q *CustomerQuery) QueryGroups() *CustomerGroupQuery {
	query := (&CustomerGroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *CustomerQuery) WithAddresses(opts ...func(*CustomerAddressQuery)) *CustomerQuery {
	query := (&CustomerAddressClient{config: _q.config}).Query()
...
func (_q *CustomerQuery) WithGroups(opts ...func(*CustomerGroupQuery)) *CustomerQuery {
	query := (&CustomerGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(customer.FieldCreatedBy).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Customer.Query().
//		Select(customer.FieldCreatedBy).
//		Scan(ctx, &v)
...
func (_q *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	...
	if query := _q.withAddresses; query != nil {
		if err := _q.loadAddresses(ctx, query, nodes,
			func(n *Customer) { n.Edges.Addresses = []*CustomerAddress{} },
			func(n *Customer, e *CustomerAddress) { n.Edges.Addresses = append(n.Edges.Addresses, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withGroups; query != nil {
		if err := _q.loadGroups(ctx, query, nodes,
			func(n *Customer) { n.Edges.Groups = []*CustomerGroup{} },
			func(n *Customer, e *CustomerGroup) { n.Edges.Groups = append(n.Edges.Groups, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CustomerQuery) loadAddresses(ctx context.Context, query *CustomerAddressQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *CustomerAddress)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(customeraddress.FieldCustomerID)
	}
...
func (_q *CustomerQuery) loadGroups(ctx context.Context, query *CustomerGroupQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *CustomerGroup)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Customer)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.CustomerGroup(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.GroupsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.customer_groups
		if fk == nil {
			return fmt.Errorf(`foreign-key "customer_groups" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "customer_groups" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
