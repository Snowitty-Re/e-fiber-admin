...
func (_q *AdminUserQuery) QueryRoles() *RoleQuery {
...
func (_q *AdminUserQuery) WithRoles(opts ...func(*RoleQuery)) *AdminUserQuery {
	query := (&RoleClient{config: _q.config}).Query()
...
//		GroupBy(adminuser.FieldCreatedBy).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.AdminUser.Query().
//		Select(adminuser.FieldCreatedBy).
//		Scan(ctx, &v)
...
func (_q *AdminUserQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*AdminUser, init func(*AdminUser), assign func(*AdminUser, *Role)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*AdminUser)
	nids := make(map[int]map[*AdminUser]struct{})
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(role.FieldID), joinT.C(adminuser.RolesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(adminuser.RolesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(adminuser.RolesPrimaryKey[1]))
